...
export const viewDataAttendance = async (req, res) => {
  ...
  try {
    ...
    resultDataAttendance = attendanceData.map((attendance) => {
      ...
      const employeePosition = attendance.position_name;
...
//     const data_position_name = await DataAttendance.findOne({
//       where: {
//         position: position,
//       },
//     });
//     console.log(data_position_name);
...
export const createAttendanceData = async (req, res) => {
  ...
  try {
    ...
    // Assuming position information is in the JobData table
    const jobData = await PositionData.findOne({
      where: {
        id: data_employee_name.id,
      },
    });
    // console.log("jobData:", jobData);
    // Check if job data not found
    if (!jobData) {
      return res.status(404).json({ msg: "Job data not found for the employee" });
    }
    ...
    if (!already_existing_name) {
      ...
      await DataAttendance.create({
        ...
        employee_name: data_employee_name.employee_name,
        gender: gender,
        position: jobData.position, // Use position from JobData
        present: present,
        sick: sick,
        absent: absent,
      });
...
export const getEmployeeData = async () => {
  ...
  try {
    ...
    resultEmployeeData = employeeData.map((employee) => {
      ...
      return { id, nik, employeeName, gender, employeePosition };
    });
  } catch (error) {
    console.error(error);
  }
...
export const getPositionData = async () => {
  ...
  try {
    ...
    const positionData = await PositionData.findAll({
      attributes: ["position", "base_salary", "transport_allowance", "meal_allowance"],
      distinct: true,
    });
...
//     const deductionEmployee = resultAttendanceData.map((attendance) => {
//       // Deductions for absent and sick
//       const deductionAbsent = resultDeductionData
//         .filter((ded) => ded.deductionName.toLowerCase() === "absent")
//         .reduce((total, ded) => total + ded.deductionAmount * attendance.absent, 0);
//       console.log(deductionAbsent);

//       const deductionSick = resultDeductionData
//         .filter((ded) => ded.deductionName.toLowerCase() === "sick")
//         .reduce((total, ded) => total + ded.deductionAmount * attendance.sick, 0);
//       console.log(deductionSick);
//       return {
//         year: attendance.year,
//         month: attendance.month,
