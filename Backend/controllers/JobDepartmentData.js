    ...
    try {
        ...
        if (req.accessRights === "admin") {
            response = await JobData.findAll({
                attributes: ['id', 'position', 'base_salary', 'transport_allowance', 'meal_allowance'],
                include: [{
                    model: EmployeeData,
            ...
            const { position, base_salary, transport_allowance, meal_allowance } = req.body;
            await JobData.update({
                position, base_salary, transport_allowance, meal_allowance
            }, {
                where: {
