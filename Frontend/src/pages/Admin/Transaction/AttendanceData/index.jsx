...
const dataAttendance = () => {
    ...
    const filtereddataAttendance = dataAttendance.filter((dataAttendance) => {
        const isMatchMonth =
            filterMonth === "" ||
            dataAttendance.month.toLowerCase().includes(filterMonth.toLowerCase());
        const isMatchYear =
            filterYear === "" || dataAttendance.year.toString() === filterYear;
        const isMatchName =
            filterName === "" ||
            dataAttendance.employee_name.toLowerCase().includes(filterName.toLowerCase());
    ...
    return (
        <Layout>
            ...
            <div className='rounded-sm border border-stroke bg-white px-5 pt-2 pb-2.5 shadow-default dark:border-strokedark sm:px-7.5 xl:pb-10 mt-6'>
                ...
                <div className='flex flex-col md:flex-row md:justify-between items-center mt-4'>
                    ...
                    <div className='w-full md:w-1/2 flex justify-center md:justify-end'>
                        <div className='w-full md:w-auto'>
                            <Link to='/attendance-data/attendance-form/add'>
                                <ButtonOne>
                ...
                <div className="bg-gray-2 text-left dark:bg-meta-4 mt-6">
                    {filtereddataAttendance.slice(startIndex, endIndex).reduce((uniqueEntries, data) => {
                        ...
                        <h2 className="px-4 py-2 text-black dark:text-white" key={`${data.month}-${data.year}`}>
                            Displaying Employee Attendance Data for the Month:
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <div className='flex items-center space-x-3.5'>
                                                <Link className='hover:text-black'
                                                    to={`/attendance-data/attendance-form/edit/${data.id}`}
                                                >
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Displaying {startIndex + 1}-{Math.min(endIndex, filtereddataAttendance.length)} of {filtereddataAttendance.length} Employee Attendance Data
                        </span>
                    </div>
