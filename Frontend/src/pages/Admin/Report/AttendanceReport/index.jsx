...
import Swal from 'sweetalert2';
import { BiSearch } from 'react-icons/bi';
import { fetchReportAbsentByMonth, fetchReportAbsentByYear, getMe } from '../../../../config/redux/action';
...
const AttendanceReport = () => {
    ...
    const handleSearch = async (event) => {
        ...
        await Promise.all([
            dispatch(fetchReportAbsentByYear(selectedYear, () => (yearDataFound = true))),
            dispatch(fetchReportAbsentByMonth(selectedMonth, () => (monthDataFound = true))),
        ]);
        setShowMessage(true);
        ...
        if (yearDataFound && monthDataFound) {
            setShowMessage(false);
            navigate(
                `/attendance/report/print-page?month=${selectedMonth}&year=${selectedYear}`
            );
        } else {
            setShowMessage(false);
            Swal.fire({
    ...
    return (
        <Layout>
            <Breadcrumb pageName='Employee Attendance Report' />
            ...
            <div className='sm:grid-cols-2'>
                <div className='flex flex-col gap-9'>
                    <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
                        <div className='border-b border-stroke py-4 px-6.5 dark:border-strokedark'>
                            <h3 className='font-medium text-black dark:text-white'>
                                Filter Employee Attendance Report
                            </h3>
                        </div>
                        <form onSubmit={handleSearch}>
                            {showMessage && (
